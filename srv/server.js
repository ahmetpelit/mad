    ...
    if (req.authInfo.checkScope('$XSAPPNAME.User')) {
        ...
        if (!serviceBinding.hasOwnProperty('error')) {
            ...
            hdbext.createConnection(hanaOptions, function (err, db) {
                ...
                // insert
                let sqlstmt = `INSERT INTO "mad.db::tenantInfo" ("tenant", "timeStamp") VALUES('` + services.registry.appName + `-` + req.authInfo.getSubdomain() + `-` + req.authInfo.getZoneId() + `', CURRENT_TIMESTAMP)`;
                db.exec(sqlstmt, function (err, results) {
                    ...
                    // query
                    sqlstmt = 'SELECT * FROM "mad.db::tenantInfo"';
                    db.exec(sqlstmt, function (err, results) {
                        if (err) {
                            console.log(err.message);
                            res.status(500).send(err.message);
